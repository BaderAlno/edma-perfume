...
interface ProductCardProps {
    ...
    /** When true, Add to Cart button is replaced with an "out of stock" message */
    outOfStock?: boolean;
    /** If a product is low on stock, this represents the exact quantity left */
    lowStockQuantity?: number | null;
    /** Optimize loading for above-the-fold cards */
    priority?: boolean;
}
            ...
            >
                ...
                <div className="group relative flex flex-col h-full bg-gradient-to-b from-[#2A1810] to-[#1A0E08] border border-[#D4AF37]/15 overflow-hidden transition-[border-color] duration-700 ease-out hover:border-[#D4AF37]/40">
                    ...
                    >
                        ...
                        {idx < 2 && (
                            <div className={language === 'ar' ? "absolute top-4 right-4 px-4 py-1.5 bg-gradient-to-r from-[#D4AF37] to-[#C9A84C] text-[#0A0806] text-xs font-arabic font-bold rounded-full shadow-[0_0_15px_rgba(212,175,55,0.4)] z-10 pointer-events-none" : "absolute top-4 left-4 px-4 py-1.5 bg-gradient-to-r from-[#D4AF37] to-[#C9A84C] text-[#0A0806] text-[10px] uppercase font-bold tracking-widest rounded-full shadow-[0_0_15px_rgba(212,175,55,0.4)] z-10 pointer-events-none"}>
                        ...
                        <div className="flex flex-col gap-4 mt-4">
                                ...
                                {outOfStock ? (
                                    <>
                                        ...
                                        <span
                                            className={
                                                language === "ar"
                                                    ? "font-arabic text-xs text-[#EBE5D9]/40 tracking-wide"
                                                    : "font-sans text-[10px] uppercase tracking-wider text-[#EBE5D9]/40"
                                            }
                                        >
                                            {language === "ar" ? "نفذ المخزون" : "Out of Stock"}
                                        </span>
                                    </>
                                ) : lowStockQuantity !== null && lowStockQuantity > 0 ? (
                                    <>
                                        ...
                                        <span className={language === 'ar' ? "font-arabic text-xs font-medium text-red-400 tracking-wider" : "font-sans text-[10px] font-bold text-red-400 uppercase tracking-widest"}>
                                            {language === 'ar' ? `الكمية محدودة - تبقى ${lowStockQuantity} فقط!` : `Limited Stock - Only ${lowStockQuantity} Left!`}
                                        </span>
                                    </>
                                ) : (
                                    <>
                            ...
                            <div className="flex gap-4">
                                <button
                                    ...
                                    className={
                                        language === "ar"
                                            ? "flex-1 text-center py-4 font-arabic text-sm font-medium tracking-wide transition-all duration-300 ease-out rounded-full disabled:opacity-40 disabled:cursor-not-allowed hover:scale-[1.03] hover:shadow-[0_0_30px_rgba(212,175,55,0.4)] active:scale-[0.98] disabled:hover:scale-100 disabled:hover:shadow-none"
                                            : "flex-1 text-center py-4 font-sans text-xs uppercase font-semibold tracking-[0.15em] transition-all duration-300 ease-out rounded-full disabled:opacity-40 disabled:cursor-not-allowed hover:scale-[1.03] hover:shadow-[0_0_30px_rgba(212,175,55,0.4)] active:scale-[0.98] disabled:hover:scale-100 disabled:hover:shadow-none"
                                    }
                                    style={{
                                        ...
                                        color: outOfStock ? "#EBE5D9" : "#1A0E08",
                                ...
                                >
                                    {outOfStock
                                        ? (language === "ar" ? "نفذ المخزون" : "Out of Stock")
                                        : added
                                            ? tSection.added[language]
                                            : tSection.addToCart[language]
                                    }
                                </button>
                            </div>
                        </div>
